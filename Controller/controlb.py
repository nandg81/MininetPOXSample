...
def _handle_ConnectionUp(event):
	...
	#Since the various hosts are in various subnets, the switch CS acts as a router and modifies the packet header fields as part of the rule actions, so that the packet gets forwarded properly from cs
		...
		log.debug("Flow rule for destination 10.0.1.10 added at cs (s5)")
		
		#Similar to the above rule for IP destination 10.0.1.10 above, the rules for all other IP destinations are added below
		...
		event.connection.send(msg)
		log.debug("Flow rule for destination 10.0.1.20 added at cs (s5)")
		...
		event.connection.send(msg)
		log.debug("Flow rule for destination 10.0.1.30 added at cs (s5)")
		...
		event.connection.send(msg)
		log.debug("Flow rule for destination 10.0.2.10 added at cs (s5)")
		...
		event.connection.send(msg)
		log.debug("Flow rule for destination 10.0.4.10 added at cs (s5)")
...
def _handle_PacketIn(event):
	...
	#ARP requests need to be handled at the controller in _handle_PacketIn
	if(dpid==5):
